...
SudokuCell::SudokuCell(int x, int y, int num, int _min, int _max) :
    NumSet(x, y, (1 + 7 + gout.twidth(int2str(9)) + 7 + 1), h, num, _min, _max), isFocused(false) {}

void SudokuCell::draw() const {
    gout << move_to(_x, _y) << color(0, 0, 0) << box(_size_x, _size_y) // Keret megrajzolása
    << move_to(_x + 1, _y + 1) << color(255, (isFocused ? 0 : 255), (isFocused ? 0 : 255)) << box(_size_x - 2, _size_y - 2) // Fókusz keret
    << move_to(_x + 2, _y + 2) << color(255, 255, 255) << box(_size_x - 4, _size_y - 4); // Belsõ fehérség
    if (num != 0)
        gout << move_to(_x + 8, _y + h - 7) << color(0, 0, 0) << text(int2str(num)); // Szám berajzolása
}
